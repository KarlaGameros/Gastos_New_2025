  ...
  <q-table
    ...
    :rows="
      props.tipo == 'pendientes'
        ...
        ? list_Solicitudes_RF_Reembolso_Aprobado.filter(
            (x) =>
              x.estatus != 'Pagada' &&
              !x.pagado &&
              x.aprobado_Repsonsable_Area &&
...
const verSolicitud = async (id) => {
  ...
  solicitudesGCReemRFStore.actualizarModalDevengado(true);
  $q.loading.hide();
};
...
const pagar = async (row) => {
  ...
  if (row.estatus_Recepcion_Administracion == "Pendiente_Recepcion") {
    $q.dialog({
      title: "Recibir solucitud de reembolso antes de pagar",
...
const concluir = async (row) => {
  Swal.fire({
    title: "Conclusión de solicitud de reembolso",
